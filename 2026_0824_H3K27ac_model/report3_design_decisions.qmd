---
title: "Architecture and input decisions: predicted p300 improves ABC in the training cell type, and not on transfer"
date: "2026-09-05"
status: "draft"
format:
  html:
...
**Method.** (Scripts generated from the existing submit scripts with exactly three edits) `n_layers`, the derived trimming sum, and the output directory, so the narrow models are a matched baseline.
...
- Two variants per direction: offset from the target's own ATAC model (an optimistic upper bound, since that model requires target H3K27ac), and offset from the transferred source ATAC model (deployable). (Headline metric is correlation with *observed* H3K27ac in the target) the quantity the application needs, reported on the top signal quintile.
...
## No architecture transfers better than the simplest
...
directories are `2026_0529_multimodal_p300_model/models/atac` (multimodal, despite the name), and `models/atac_only`. Target and control tracks are
...
## Predicted p300 improves ABC in the training cell type, and not on transfer
...
## DNase beats ATAC as the accessibility input, and it is the first H3K27ac model to clear the floor on transfer
...
## GC-matching the training negatives changes nothing
