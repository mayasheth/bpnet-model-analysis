...
## Sequence carries an accessibility-independent component — but only if you train for it
...
**A:** It does, and the earlier answer was wrong: a sequence model trained on the total signal captures almost none of the accessibility residual (*r* = 0.149 [0.076, 0.222]), while the same architecture on the same sequence-only input, trained directly on `observed − atac_pred`, reaches 0.459 [0.421, 0.496] — but the residual objective helps *only* when the model cannot see accessibility.
...
**Figure 5 | The residual objective helps only when the model cannot see accessibility.** Baseline throughout is the held-out prediction of the accessibility-only *model* (`atac5p_hw500_clw10`), not the raw ATAC track, so the residual is what accessibility genuinely cannot explain rather than what a linear read of the track misses. Colour encodes input modality (red = sequence only, purple = sequence + ATAC, blue = ATAC only); hatching encodes the training objective (hatched = trained on `observed − atac_pred` rather than on the total signal). Pairs of bars sharing a colour therefore have **identical inputs** and differ only in what they were trained to predict. **a**, Residual Pearson *r* = `r(observed − atac_pred, model_pred − atac_pred)`, mean over the 5 chromosome-holdout folds with *t*-based 95% CI; points are individual folds. Sequence on total signal 0.149 [0.076, 0.222]; sequence on residual 0.459 [0.421, 0.496]; sequence + ATAC on total signal 0.551 [0.510, 0.592]; sequence + ATAC on residual 0.514 [0.473, 0.556]; ATAC on residual −0.003 [−0.065, 0.060]. Because folds are paired across models, paired differences are far tighter than these marginal intervals: residual training **gains** +0.310 [0.224, 0.395] (*p* = 5×10⁻⁴) for sequence-only but **costs** −0.037 [−0.039, −0.034] (*p* < 10⁻⁴) for sequence + ATAC, the latter being −0.035 to −0.039 in every single fold. **b**, Residual *r* by quintile of |true residual|, i.e. by how wrong accessibility is for that element, pooled over folds. The four informative models rise monotonically — 0.033–0.091 where accessibility is nearly right, to 0.261–0.712 where it is most wrong — while the ATAC control stays flat at zero across all five strata. **c**, Incremental *R²* over the ATAC-only model scored against the **observed** signal, all elements and top signal quintile. Sequence-on-total is substantially worse than ATAC alone (−0.418 [−0.467, −0.368]); sequence-on-residual +0.071 [0.058, 0.085]; sequence + ATAC on total +0.102 [0.087, 0.117]; on residual +0.090 [0.077, 0.104]; ATAC control −0.000 [−0.001, 0.001]. n = 58,655 element–fold observations over 5 folds. Source: `results/residual_grid_{per_fold,fold_summary,residual_evaluation}.tsv`.
</details>

**Three things follow, and the full 2×2 of {input} × {objective} is what separates them.**
...
Residual training's remaining value is therefore interpretive rather than predictive: its attributions are forced onto signal accessibility cannot supply, which is what the motif-syntax work needs and what the multimodal model's attributions cannot cleanly separate.
