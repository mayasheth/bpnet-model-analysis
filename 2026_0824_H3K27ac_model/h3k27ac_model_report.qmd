...
- **Transfer asymmetry reflects cell-type difficulty, not a sequence-specific failure.** GM12878-trained models score higher on K562 than in their own cell type for every modality (Fig. 7), and the ATAC-only gap is explained by weaker ATAC–H3K27ac coupling in GM12878 (Fig. 8). Sequence remains weak everywhere (15–41% of ceiling), and its margin over accessibility shrinks on transfer in both directions.
...
The training objective, not the input, is what limits the sequence-only model: told to predict total signal it spends its capacity on the large accessibility-correlated component and leaves the residual alone.
...
**The effect belongs to the objective, not to K562.** It reproduces in GM12878, where weaker ATAC–H3K27ac coupling (0.409 vs 0.510) and a higher ceiling (0.832 vs 0.760) pull in opposite directions; the multimodal cost lands within 0.002 of itself. Absolute levels are lower throughout, consistent with GM12878 being harder to predict (Fig. 7), but every ordering and sign holds.

Residual training is therefore an interpretability tool, not a predictive one. Its attributions are forced onto signal accessibility cannot supply — what the motif work needs, and what multimodal attributions cannot cleanly separate — and being a property of the objective, it needs no per-cell-type re-testing.
...
**A:** The multimodal one — not because it predicts better, but because it is indistinguishable from the residual alternative on the top signal quintile and simpler, with no offset model to ship alongside it.
...
**Sequence adds on transfer.** Every sequence-containing model beats transferred ATAC-only by 0.04–0.06 on the top quintile. That margin, not the choice of objective, is what sequence buys.

**Accessibility is portable but must be measured locally.** Transferring the ATAC baseline rather than fitting it in the target costs only ~0.017 (0.494 → 0.477), while sequence-only transfer is useless (0.147, 0.317).

**A shallow target H3K27ac library would be worth more than its size suggests.** The residual design beats multimodal when the accessibility baseline is fitted in the target rather than transferred (+0.005 to +0.011 on all elements, *p* < 0.01 both directions). Fitting that baseline needs only an ATAC-only model — far less data than training a full model — so even a modest H3K27ac library in a new cell type would make the residual architecture preferable.
...
**Figure 8 | Model-free coupling between accessibility and H3K27ac, and what it implies for transfer.** Hexbin of log1p summed ATAC against log1p summed H3K27ac over identical ±500 bp element-centred windows in K562 (n = 150,519); colour is element density, log scale. No model involved — the direct correlation between the two assays, and therefore a reference for what any accessibility-only predictor could reach. Top-quintile Pearson: **K562 0.510, GM12878 0.409**. Against that reference:
...
The ATAC-only model does not degrade across cell types — it exceeds the raw coupling in both, by a wider margin in GM12878 — so its lower absolute score there is a property of the assay pair, not the model. Sequence-only falls 0.360 → 0.146 with no analogous denominator to absorb it. This sharpens the transfer conclusion rather than weakening it: accessibility transfers, only sequence collapses. Both cell types use the same 5′ target processing and window. Source: `results/atac_vs_h3k27ac_by_celltype.tsv`.
</details>
...
- **Score architecture changes on transfer, not held-out chromosomes** — Fig. 7 shows in-cell-type performance is a poor guide to generalisation here.
- **Between-fold variability dominates.** Fold `sd` is 0.041–0.046 for single-input models, giving CI half-widths near 0.05 against a run-to-run variance of 0.018. Differences under ~0.05 need more folds or elements, not more seeds — and paired within-fold comparison where the models see identical held-out chromosomes.
