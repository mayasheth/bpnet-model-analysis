...
## A wider receptive field separates dead from active elements and adds nothing among active ones
...
**A:** It gains +0.043 in K562 and +0.045 in GM12878 on all elements and nothing on the elements carrying the mark (−0.006 and +0.010, p = 0.53 in both, sign flipping between cell types), so the extra context resolves whether a region is active rather than how active it is.
...
**Figure 10 | Quadrupling the receptive field moves the all-element correlation and leaves the top quintile flat.** Sequence-only models, `n_layers` 8 (trimming 557, 2,114 bp input, ~1.1 kb receptive field) against `n_layers` 10 (trimming 2,093, 5,186 bp input, ~4.2 kb). Grey lines are individual folds, red the mean; the comparison is paired within fold because fold `sd` here is 0.041–0.046, several times the effect. **a**, K562: all elements 0.494 → 0.537 (paired +0.043, p = 0.003), top quintile 0.381 → 0.375 (paired −0.006, p = 0.53, n.s.). **b**, GM12878: all elements 0.475 → 0.520 (paired +0.045, p = 0.005), top quintile 0.221 → 0.232 (paired +0.010, p = 0.53, n.s.). Both models are scored on the intersection of regions each could accept, since the 5,186 bp window drops elements within 2,593 bp of a chromosome end that the narrower one keeps. Everything except `n_layers` and the derived window geometry is byte-identical between the two arms. Source: `results/wide_seqonly_{k562,gm12878}_per_fold.tsv`.
</details>
...
for M in sequence multimodal; do for F in 0 1 2 3 4; do
    sbatch scripts/1.13.submit_training_5prime_wide.sh $M $F
    sbatch scripts/1.14.submit_training_gm12878_wide.sh $M $F
done; done
sbatch scripts/2.17.submit_regression_and_wide.sh wide_seqonly
```
...
- **Wider receptive field — answered, and not worth the GPU time for this target.** ~4.2 kb gains +0.043 and +0.045 on all elements and nothing on the top quintile, replicated in two cell types (Fig. 10). Revisit only if the application becomes on/off classification, where the all-element gain is real. The multimodal arm is still training and is expected to gain less, since accessibility already supplies long-range context.
- **Nucleosome-resolution profile target — now supported by a measured ceiling.** At 1 bp a perfect model caps at 0.21; binning to 50 bp raises the ceiling to 0.72 (Fig. 11). The choice of width is a judgement call rather than a data-determined one, since the curve has no elbow; 50 bp keeps 20 bins across the window while capturing most of the available gain. Note that every metric in this report comes from the counts head, so binning changes the reported numbers only through the shared trunk.
- **Fragment-size-stratified accessibility channels — tracks built, training queued.** Five channels `[all, sub ≤139, mono 140–329, di 330–620, poly ≥621]`, all single-base insertion counts, which sum to the flat track exactly (0 discrepancy in 545.7 million insertions), so the input is a strict superset of the present baseline and cannot regress. The Tn5 shift was measured against the existing track rather than assumed (*r* = 1.0000 at +4/−5). The bins in Fig. 9 are superseded: they were full-fragment coverage, the wrong convention for a 5′ model, and covered half the fragments.

**Standing cautions**

- **Stratify.** Most elements carry no H3K27ac, so all-element correlations are dominated by the dead-versus-active contrast — four misleading readings so far, most recently the receptive-field result in Fig. 10, where a significant +0.043 on all elements is a null on the elements that carry the mark.
