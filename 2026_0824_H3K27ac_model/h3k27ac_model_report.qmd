...
**A:** It does, and the earlier answer was wrong: a sequence model trained on the total signal captures almost none of the accessibility residual (*r* = 0.149 [0.076, 0.222]), while the same architecture on the same sequence-only input, trained directly on `observed − atac_pred`, reaches 0.459 [0.421, 0.496].
...
![](figures/fig5_residual_beyond_accessibility.png)

<details>
<summary>Figure 5 legend</summary>

**Figure 5 | Sequence carries an accessibility-independent component that a total-signal objective fails to surface.** Baseline throughout is the held-out prediction of the accessibility-only *model* (`atac5p_hw500_clw10`), not the raw ATAC track, so the residual is what accessibility genuinely cannot explain rather than what a linear read of the track misses. Colour encodes input modality (red = sequence only, purple = sequence + ATAC); hatching encodes the training objective (hatched = trained on `observed − atac_pred`). The two red bars therefore have **identical inputs** and differ only in what they were trained to predict. **a**, Residual Pearson *r* = `r(observed − atac_pred, model_pred − atac_pred)`, mean over the 5 chromosome-holdout folds with *t*-based 95% CI; points are individual folds. Sequence trained on total signal 0.149 [0.076, 0.222]; same input trained on the residual 0.459 [0.421, 0.496]; sequence + ATAC 0.551 [0.510, 0.592]. **b**, Residual *r* by quintile of |true residual|, i.e. by how wrong accessibility is for that element, pooled over folds. All three rise monotonically — 0.033 / 0.067 / 0.091 in the quintile where accessibility is nearly right, to 0.261 / 0.638 / 0.712 where it is most wrong — which is the behaviour a real effect should show and an artifact of the metric should not. **c**, Incremental *R²* over the ATAC-only model scored against the **observed** signal, for all elements and the top signal quintile; this metric cannot be inflated by the artifact described in Methods. Sequence-on-total is substantially *worse* than ATAC alone (−0.418 [−0.467, −0.368]; −0.156 on the top quintile), residual-trained is +0.071 [0.058, 0.085] (+0.115 [0.075, 0.154]), and sequence + ATAC +0.102 [0.087, 0.117] (+0.169 [0.131, 0.207]). n = 58,655 element–fold observations over 5 folds. Source: `results/residual5p_per_fold.tsv`, `results/residual5p_fold_summary.tsv`, `results/residual5p_residual_evaluation.tsv`.
</details>
...
- *Residual training is nonetheless not the better predictor.* The jointly-trained sequence + ATAC model captures **more** of the residual (0.551) than explicit residual training does (0.459), and more incremental *R²*. Joint training already exploits the accessibility-independent component, and exploits it better. So residual training's value here is diagnostic — it demonstrates the component exists — and interpretive: its attributions are forced onto signal accessibility cannot supply, which is what the motif-syntax work needs and what the multimodal model's attributions cannot cleanly separate.
...
**The residual metric has a built-in artifact, and it was explicitly excluded.** Because `true_resid = obs − atac_pred`, *any* prediction that merely anti-correlates with `atac_pred` earns positive residual *r* for free, without carrying information about what accessibility missed — and a sequence model can partially predict accessibility, so the channel is live rather than hypothetical. Three checks rule it out for the residual-trained model: its output correlates only −0.090 [−0.104, −0.076] with `atac_pred`; partialling `atac_pred` out of both sides leaves the score unchanged (0.459 → 0.459, versus 0.149 → 0.205 for the sequence-on-total model, which is the more `atac_pred`-entangled of the two at +0.477); and the incremental *R²* in panel **c** is scored against the observed signal, which cannot be faked out of sample.
...
# per-fold CIs + the artifact controls, then the figure
pixi run python scripts/2.8.residual_perfold_and_artifact.py
