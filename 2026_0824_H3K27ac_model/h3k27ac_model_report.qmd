...
> **Superseded on 2026-09-05.** Split into three reports ordered by how often they change:
> [Report 1, data characterisation](report1_data_characterisation.html),
> [Report 2, evaluation methodology](report2_evaluation_methodology.html) and
> [Report 3, design decisions](report3_design_decisions.html). Nothing here is newer than
> those three; every section was moved rather than rewritten. Kept for the record only.
...
- **Track construction matters at the resolution level.** ATAC and H3K27ac are spatially offset — ATAC peaks on the element centre, H3K27ac on flanking nucleosomes at ±250 bp and is far broader (Fig. 2). H3K27ac is still at 28% of maximum at ±2 kb, the edge of the profiled window, so its extent exceeds the model's ~1.1 kb receptive field by an unmeasured margin. Relatedly, switching the accessibility input to ChromBPNet-style 5′ insertion counts gains the ATAC-only model +0.032 (K562) and +0.023 (GM12878) on the top quintile while leaving sequence + ATAC unchanged (Fig. 6) — within a cell type the change is one of resolution, and only a model without sequence feels it. Widening the receptive field to ~4.2 kb pays off only for the model that holds accessibility: sequence + ATAC gains +0.027 (K562) and +0.014 (GM12878) on the elements carrying the mark, while sequence alone gains nothing there (Fig. 10). Enriching the accessibility input pays off too — five fragment-size channels in place of flat coverage gain +0.0135 on the same elements (Fig. 12) — so both of the architecture changes that have worked act on the accessibility side. Separately, averaging each prediction with its reverse complement at inference is free and helps every model, most where sequence matters most (+0.0162 sequence only, +0.0081 sequence + ATAC, +0.0016 and not significant for ATAC only; Fig. 13). At the other end of the resolution scale, the base-resolution profile is nearly unreproducible — inter-replicate ceiling 0.21 at 1 bp against 0.72 at 50 bp binning (Fig. 11) — which caps what the profile head can ever score rather than showing that it fails.
...
**Q:** Does widening the receptive field from ~1.1 kb to ~4.2 kb improve H3K27ac prediction, given that the mark extends well past ~1.1 kb (Fig. 2)?
...
<details>
<summary>Figure 12 legend</summary>

**Figure 12 | Five fragment-size accessibility channels beat one flat channel on the elements carrying H3K27ac.** K562 sequence + ATAC, 5 folds, paired within fold. Accessibility input is `[all, sub ≤139, mono 140–329, di 330–620, poly ≥621]` bp, every channel a single-base Tn5 insertion count. **a**, All elements 0.876 → 0.878 (paired +0.0024, p = 0.030). **b**, Top quintile 0.690 → 0.703 (paired +0.0135, p = 0.0034), every fold rising. Grey lines individual folds, purple the mean. The four length bins sum to the flat 5′ track exactly — zero discrepancy across 545,661,218 insertions and zero largest single-base difference over 2 Mb — so the input is a strict superset of the baseline and the gain is added information rather than a changed input definition. Bin edges sit in the troughs of the fragment-length distribution (Fig. 9); the earlier `sub`/`mono` channels used full-fragment coverage and covered half the fragments, and are superseded. The Tn5 shift needed to build them from the paired-end BAMs was measured against the existing track rather than assumed (*r* = 1.0000 at +4/−5). Source: `results/fragchan_k562_per_fold.tsv`.
</details>
...
- **Wider receptive field — worth adopting for the multimodal model.** ~4.2 kb gains +0.027 (K562) and +0.014 (GM12878) on the top quintile, significant and same-signed in both cell types, and lifts the accessibility residual from 0.502 to 0.547 and 0.397 to 0.469 (Fig. 10). Sequence alone gains only on all elements. An ATAC-only arm is training: if it reproduces the gain, the extra context is being used entirely as accessibility neighbourhood and sequence is irrelevant to it.
- **Nucleosome-resolution profile target — supported by a measured ceiling, but the head is not failing.** At 1 bp a perfect model caps at 0.21; binning to 50 bp raises the ceiling to 0.72 (Fig. 11). On the top quintile the models reach `profile_pearson` of 0.114 (ATAC), 0.144 (sequence) and 0.171 (sequence + ATAC), so most of the small reproducible signal is already captured and the case for binning is that it raises the ceiling rather than that the current head learns nothing. Treat the ratio of those two numbers as approximate: the ceiling excludes elements flat in either replicate while `profile_pearson` scores them as zero, which deflates the model side. Every other metric in this report comes from the counts head, so binning moves them only through the shared trunk.
- **Fragment-size-stratified accessibility channels — done, and worth adopting.** +0.0135 on the top quintile, p = 0.0034 (Fig. 12). Five channels that sum to the flat track exactly, so the gain is added information. Open: whether it combines with the wider receptive field, since both act on the accessibility side and might be reading the same neighbourhood structure twice; and GM12878 replication, which needs its paired-end BAMs downloaded because fragment length lives in TLEN.

**Standing cautions**

- **Stratify, and check every input mode.** Most elements carry no H3K27ac, so all-element correlations are dominated by the dead-versus-active contrast — four misleading readings so far, including sequence-only in Fig. 10, where a significant +0.043 on all elements is a null on the elements that carry the mark. Fig. 10 also caught the converse: the sequence and multimodal arms of the same architecture change point opposite ways on the top quintile, so a conclusion drawn from whichever arm finishes first can invert.
...
Model is `MultiModalBPNet` (`scripts/multimodal_bpnet.py`), a BPNet variant with a dilated-convolution trunk, a profile head trained with multinomial negative log-likelihood, and a counts head trained with log1p mean-squared error; total loss is `profile_loss + w · count_loss`. All results here concern the counts head. Training uses `n_layers = 8` (a ~1.1 kb receptive field), a ±500 bp counting window with the required 2,114 bp input window, `w = 10`, reverse-complement augmentation, chromosome-holdout 5-fold cross-validation, and a 50,000-window cap on the GC-matched negative pool. Figure 10 additionally uses `n_layers = 10`, whose trimming of 2,093 requires a 5,186 bp input window for the same ±500 bp output. Windows are centred on candidate-element midpoints, because the H3K27ac summit sits on a flanking nucleosome (Fig. 2).
