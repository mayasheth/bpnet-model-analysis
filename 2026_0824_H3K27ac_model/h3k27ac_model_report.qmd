...
**A:** The multimodal one, not because it predicts better than the residual-objective alternative but because it is indistinguishable from it on the top signal quintile and simpler, with no offset model to ship alongside it.
...
- **Sequence genuinely adds on transfer.** Every sequence-containing model beats transferred ATAC-only by 0.04–0.06 on the top quintile. That margin, not the choice of objective, is what the sequence component buys.
- **Accessibility is largely portable, and must be measured locally.** Transferring the ATAC baseline instead of fitting it in the target costs only ~0.017 (0.494 → 0.477), but sequence-only transfer is useless (0.147, 0.317) — the accessibility measurement in the target cell type is doing the heavy lifting either way.
- **A little target H3K27ac would be worth more than its size suggests.** The residual design *does* beat multimodal when the accessibility baseline is fitted in the target rather than transferred (+0.005 to +0.011 on all elements, *p* < 0.01 both directions). Fitting that baseline needs only an ATAC-only model, i.e. a modest H3K27ac library — not enough to train a full model on. If a shallow H3K27ac library in a new cell type is feasible, the residual architecture becomes preferable.
...
**A cautionary result about strata.** On *all elements* the picture looks tidy and significant: residual-multimodal beats multimodal by +0.005 (K562→GM, *p*=0.009) and +0.011 (GM→K562, *p*=0.003) with a locally fitted baseline, and loses by −0.005 (*p*=0.012) and −0.005 (*p*=0.022) when the baseline is transferred — a clean sign flip, all four significant. **On the top quintile none of it is resolvable** (*p* = 0.17, 0.33, 0.65, 0.76). All-elements correlation on this element set is dominated by the dead-versus-active contrast that accessibility already resolves, which is why this report stratifies everywhere. The tidy story lives in that contrast, not in the biology, and is not reported as a finding.
