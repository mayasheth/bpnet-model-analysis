...
#
# EXACTLY TWO changes vs 1.11: the negatives file, and the output directory.
#
# WHY: every model in this repository was trained with the genome-wide GC-ANNOTATED TILING
# passed straight to --negatives. That file is ChromBPNet's matching INPUT, not its output;
# the trainer reads only its first three columns and samples uniformly, so the negative pool
# sat at mean GC 0.389 against 0.466-0.593 for candidate elements. A sequence branch can
# satisfy much of that contrast with a GC detector, which is the leading hypothesis for why
# the H3K27ac sequence branch has almost no dynamic range within candidate elements.
#
# The negatives here come from bpnet-gc-background (scripts/0.28), the same tool used to
# build the p300 v3 negatives in Oct 2025, so this is the established procedure rather than
# a reimplementation.
#
# Baseline for the comparison is models/multimodal5p_accs5p_hw500_clw10, produced by 1.11
# with identical settings. Score with 2.15 and pair within fold.
set -euo pipefail
...
NEGATIVES="$PROJ/data/gc_negatives_k562_h3k27ac.bed"
FOLDS="$PROJECT_DIR/reference/hg38_five_folds.json"

OUT_DIR="$PROJ/models/${MODE}5p_accs5p_gcmatch_hw${HALF_WINDOW}_clw${COUNT_LOSS_WEIGHT}/fold${FOLD}"
