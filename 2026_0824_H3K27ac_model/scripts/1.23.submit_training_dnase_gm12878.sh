...
#
# EXACTLY TWO changes vs 1.12: the accessibility bigwig, and the output directory.
#
# Swaps DNase for ATAC as the model'''s accessibility channel. DNase couples to H3K27ac and to
# enhancer activity better than ATAC does, and observed DNase already beats observed ATAC
# inside ABC, so the question is whether that carries into a model'''s input. The answer also
# decides whether the ATAC->DNase converter is worth building: a converter only pays if DNase
# is the better representation to be in.
#
# Baseline is models/gm12878_multimodal5p_accs5p_hw500_clw10 from 1.12 with identical settings.
set -euo pipefail
...
ATAC_BW="/oak/stanford/groups/engreitz/Users/sheth/EP300_BPNet/2026_0824_H3K27ac_model/data/gm12878_dnase_5p.bw"
...
OUT_DIR="$PROJ/models/gm12878_${MODE}5p_dnase_hw${HALF_WINDOW}_clw${COUNT_LOSS_WEIGHT}/fold${FOLD}"
