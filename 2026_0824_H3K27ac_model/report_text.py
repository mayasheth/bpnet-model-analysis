...
**Why the asymmetry, and how to test it.** Both cell types are equally *predictable* locally,
so the H3K27ac explanation for its own transfer asymmetry -- that GM12878 is the harder cell
type -- cannot apply here. The remaining candidate is that the K562 EP300 experiment
(ENCSR000EGE) supports a more portable sequence model than GM12878's (ENCSR000DZG) even
though both fit their own cell type equally well; peak count, depth and signal-to-noise are
the obvious things to compare, and a third cell type would show whether K562-trained models
are portable in general or just portable to GM12878.
