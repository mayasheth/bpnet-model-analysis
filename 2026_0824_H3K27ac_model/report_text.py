...
- **Predicted p300 clears the benchmark floor; predicted H3K27ac does not.** A sequence + ATAC model predicting p300, substituted into ABC's activity term, beats the ATAC-only floor by **+0.055 AUPRC [+0.034, +0.074]** on a paired bootstrap, sign preserved in 100% of resamples, and is statistically indistinguishable from *measured* H3K27ac (-0.007 [-0.032, +0.017]). No predicted-H3K27ac arm managed this (Fig. 1).
- **Observed p300 is simply a better ABC activity term than observed H3K27ac**, by +0.038 [+0.019, +0.058], independent of any model. That widens the available headroom over the floor from 0.062 to 0.099 and is a reason to prefer p300 as the modelling target wherever it can be measured.
...
| **Predicted p300 as the ABC activity term** | **+0.055 [+0.034, +0.074]** over the floor, paired bootstrap | **adopted as the target of choice**; matches measured H3K27ac |
...
better, and is observed p300 even a better activity term to begin with?
...
the floor with a resolvable margin.
...
55% of p300's against a non-resolvable fraction of H3K27ac's. A p300-target model would be
worth preferring for either reason alone.
...
**1. Train a GM12878 p300 model and re-run the benchmark transferred.** This is now the
highest-value experiment in the project by a wide margin. Transferability to a cell type with
only ATAC is the stated goal, the K562 p300 result is the first arm to clear the floor, and
the transferred version is completely untested. EP300 data exists in GM12878 (ENCSR000DZG),
so this is five folds of training plus an existing scoring path -- no new tooling. Everything
below is lower priority until it lands.
...
def build(pull, write):
    ...
    write("report3_design_decisions.qmd",
          "Architecture and input decisions: predicted H3K27ac does not improve ABC, predicted p300 does",
          DATE,
          [R3_HEAD,
           R3_INPUTS,
