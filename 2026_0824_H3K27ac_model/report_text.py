...
- **It does not survive transfer, which is the case the project exists to serve.** A GM12878-trained p300 model applied to K562 scores **+0.009 [-0.004, +0.022]** over the floor -- interval spanning zero, indistinguishable from just using the target cell type's own ATAC. The transfer drop is **-0.046 [-0.061, -0.030]**, consuming 83% of the in-cell gain, and the transferred arm is **-0.053 [-0.074, -0.032]** below measured H3K27ac.
- **Deployment now fails for both targets, so the barrier is transfer rather than target choice.** The H3K27ac deployment arms sat at 0.455 and 0.452 against a 0.457 floor; the p300 deployment arm sits at 0.466. Target choice buys a large in-cell-type gain and nothing deployable, which is the same shape as every architecture change (F-005).
...
| **Predicted p300 as the ABC activity term, transferred** | **+0.009 [-0.004, +0.022]** over the floor; drop -0.046 [-0.061, -0.030] | **null**; no better than the target's own ATAC |
...
**One control is missing and it bounds how strongly this can be stated.** The transfer drop
conflates two things: what is lost by moving cell type, and the possibility that the GM12878
p300 model is simply weaker than the K562 one -- different experiment (ENCSR000DZG against
ENCSR000EGE), different depth, and it was trained in a different project. Separating them
needs the GM12878 p300 model scored *in GM12878*, the local-versus-transferred pairing the
H3K27ac transfer matrix used throughout. Until that runs, "the p300 gain does not transfer"
is the honest reading and "transfer specifically destroys it" is not yet established.
...
**1. Score the GM12878 p300 model in GM12878, to finish the transfer comparison.** The
transferred arm is a null (+0.009 over the floor), but the drop conflates transfer with the
possibility that the GM12878 p300 model is weaker in absolute terms -- it comes from a
different experiment and a different project. The local-versus-transferred pairing that the
H3K27ac transfer matrix used throughout is the missing control, and it is cheap: the model
exists, GM12878 EP300 data exists, and the scoring path exists. Without it the result reads as
"the gain does not transfer" and cannot be sharpened to "transfer is what destroys it".

**2. Decide whether the deployment goal is reachable at all with this design.** Both targets
now fail deployment: H3K27ac arms at 0.455 and 0.452, p300 at 0.466, against a 0.457 floor.
Every architecture change also failed to travel (F-005). Before more variants are tried it is
worth asking what a transferred model would have to get right that none so far does -- the
common structure is that all of them lean on accessibility, and accessibility is the part that
does travel, so the transferred model reduces towards the floor arm by construction. A
multi-cell-type training run is the obvious next design, and TeloHAEC plus its four conditions
is the cheapest way to get a third cell type.
