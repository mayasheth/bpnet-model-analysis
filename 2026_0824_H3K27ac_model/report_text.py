...
- **It was a plausible cause of the sequence branch's flatness, and it is not the cause.**
  A positive/negative contrast separable on GC alone is a known way to turn a BPNet-style
...
**Figure 1 | Predicted p300 clears the ABC benchmark floor in the training cell type and not on transfer.** **Headline figure.**

<details>
<summary>Full legend</summary>

**Figure 1 | p300 as the ABC activity term, K562 CRISPR benchmark, all nine arms plus
distance-to-TSS.** 10,342 element-gene pairs with 466 regulated, every arm scored on the
identical pair set. Error bars are the pipeline's **unpaired** per-predictor 95% intervals and
they overlap for every arm, resolving nothing; the paired-bootstrap delta against the floor is
printed at the right of each row, bold where resolvable and grey with `n.s.` where the
interval spans zero. Both layers are shown deliberately, because the overlapping bars are what
the eye is drawn to and they are the misleading half. Dashed lines mark the ATAC-only floor
(0.457) and observed H3K27ac (0.519). Observed p300 is the highest arm at **0.557**
(+0.099 [+0.079, +0.120] over the floor); the K562-trained predicted-p300 arm reaches
**0.512** (+0.055 [+0.034, +0.074]), matching measured H3K27ac; the **GM12878-trained arm
applied to K562 sits at 0.466** (+0.009 [-0.004, +0.022], n.s.) in a cluster at the floor
alongside the K562 ATAC-only p300 model. The H3K27ac equivalent of this figure, in which no
arm clears the floor, is Fig.~10. Source:
`CRISPR_comparison_v3/.../results/2026_0906_p300_all/performance_summary.txt`, paired deltas
from `scripts/4.17`.
</details>
...
- Transfer is strongly asymmetric and the benchmark can only test the direction that fails. K562-trained p300 applied to GM12878 keeps **+0.207 [+0.158, +0.257]** over that cell type's own accessibility model, 83% of its local advantage; GM12878-trained applied to K562 keeps **+0.006** (*p*=0.72). Both are equally strong at home (+0.313 and +0.309 over matched floors) and equalising training-signal volume does not close it, so this is neither model quality nor data volume. Observed p300 also beats observed H3K27ac as an activity term by +0.038 [+0.019, +0.058], a fact about the assay that survives every negative result below.
- Swapping DNase for ATAC as the accessibility input is the first change that survives transfer. It gains +0.037 in-cell K562 and +0.086 in-cell GM12878, and transferred it clears the target cell type's own ATAC-only floor by **+0.056 [+0.014, +0.099]** where the ATAC-input model manages +0.013 (*p*=0.25). DNase is the shallower library in both cell types, so the depth confound runs against the result.
...
| **DNase instead of ATAC as the accessibility input** | **+0.037** in-cell K562, **+0.086** in-cell GM12878, **+0.043** transferred; clears the target's floor by **+0.056** | **adopted**; the first model to clear the floor on transfer |
...
Source: `.../results/2026_0904_predicted_activity/performance_summary.txt`,
`results/paired_auprc_h3k27ac.tsv`.
</details>
...
**The paired re-test confirms the negative rather than overturning it.** These arms were
originally read off overlapping unpaired intervals, which the p300 run later showed cannot
resolve differences of this size (Report 2), so the conclusion was re-run through the same
paired bootstrap:

| comparison | delta | paired 95% CI | sign kept |
|---|---|---|---|
| observed H3K27ac - floor | +0.062 | [+0.044, +0.080] | 100% |
| best predicted arm - floor | **+0.025** | **[-0.003, +0.052]** | 95.3% |
| ATAC x predicted - floor | +0.012 | [-0.005, +0.029] | 91.0% |
| best predicted - observed H3K27ac | -0.037 | [-0.062, -0.013] | 99.9% |
| GM12878 deployment arm - floor | -0.005 | [-0.022, +0.011] | 72.7% |
...
**Figure 11 | The two error tails, what marks them, and why CTCF does not explain the
...
Fig.~1 is this result; its legend carries the full arm list. The tables below give the
numbers, including the transfer arms that the figure shows clustered at the floor.

<details>
<summary>Figure 1, restated (see Headline figure)</summary>
...
**The asymmetry is not explained by training-signal volume, and that has now been tested
directly.** Both cell types are equally *predictable* locally, so the H3K27ac explanation for
its own transfer asymmetry (that GM12878 is the harder cell type) cannot apply here. The two
EP300 experiments do differ in volume:
...
FRiP does not separate the two once replicate variance is accounted for: the within-experiment
...
worse one. So the "cleaner target" version of the hypothesis was out from the start, leaving
"more signal to learn portable features from".

**That version is now refuted.** Retraining K562 on GM12878's exact budget (30.0M mapped
reads and 21,068 peaks, `scripts/0.31` and `1.21`) costs almost nothing in-cell type and
nothing at all on transfer:

| | in-cell K562 | transferred to GM12878 |
|---|---|---|
| full-depth K562 model | 0.619 | 0.507 (+0.207 [+0.158, +0.257] over GM12878's floor) |
| depth-matched K562 model | 0.599 | **0.501 (+0.202 [+0.147, +0.257] over that floor)** |
| paired difference | -0.020 [-0.040, +0.001] (*p*=0.056) | **-0.005 [-0.021, +0.010]** (*p*=0.40) |

A K562 model trained on 41% fewer reads and 26% fewer peaks still transfers at +0.202 while
the GM12878 model, trained on that same budget, transfers at +0.006. The in-cell arm is the
control that keeps this readable: the depth-matched model is barely worse at home, so it is
not simply a weaker model.

**Peak-set geometry is also ruled out.** Both training sets are summit-centred fixed-width
(316 bp for K562, 350 bp for GM12878) and the trainer centres its window on `start + summit`,
so neither set is mis-registered against the p300 summit; GM12878's summits are the tighter of
the two (|summit - midpoint| p90 of 0 bp against 6 bp).

**What remains.** The asymmetry is a property of K562 and GM12878 as training cell types that
survives equalising volume and is not an artifact of peak geometry. Two cell types cannot
distinguish "K562-trained models are portable in general" from "K562 happens to transfer to
GM12878", so a third cell type is now required rather than merely desirable. The construction
control for these tracks is recorded in `0.31`: the full-depth rebuild matched the existing
target at *r* = 1.0000 on both strands with a sum ratio of 0.9984, which is what licenses
using the existing K562 model as the full-depth arm instead of retraining it.
...
R3_DNASE = """## DNase beats ATAC as the accessibility input, and it is the first H3K27ac model to clear the floor on transfer

**Q:** DNase couples to H3K27ac and to enhancer activity better than ATAC does, and observed
DNase already beats observed ATAC inside ABC. Does that carry into a model's input?

**A:** Yes, in every arm, and by more where DNase is the shallower library. In-cell type DNase
gains **+0.037 [+0.019, +0.056]** in K562 and **+0.086 [+0.076, +0.095]** in GM12878.
Transferred from K562 to GM12878 it gains **+0.043 [+0.003, +0.084]** over the ATAC-input
model, and it is **the first H3K27ac model in this project to clear the target cell type's own
ATAC-only floor with a resolvable margin: +0.056 [+0.014, +0.099]**, where the ATAC-input
model manages +0.013 (*p*=0.25).

| top-quintile Pearson | ATAC input | DNase input |
|---|---|---|
| in-cell K562 | 0.699 | **0.736** |
| in-cell GM12878 | 0.585 | **0.671** |
| K562 -> GM12878, transferred | 0.531 | **0.574** |
| GM12878's own ATAC-only floor | 0.518 | 0.518 |

| paired within-fold comparison | delta | 95% CI | *p* |
|---|---|---|---|
| in-cell K562, DNase - ATAC | +0.037 | [+0.019, +0.056] | 0.005 |
| in-cell GM12878, DNase - ATAC | +0.086 | [+0.076, +0.095] | <1e-4 |
| transferred, DNase - ATAC | +0.043 | [+0.003, +0.084] | 0.041 |
| **transferred DNase - GM12878's ATAC-only floor** | **+0.056** | **[+0.014, +0.099]** | **0.020** |
| transferred ATAC - that same floor | +0.013 | [-0.014, +0.041] | 0.25 |

**The accessibility residual is where the gap is widest.** Transferred, the DNase-input model
scores *r* = 0.236 [0.211, 0.262] against accessibility, the ATAC-input model 0.041
[0.003, 0.079], and the floor arm 0.026. On the metric that asks what a model adds beyond
accessibility, the transferred ATAC model adds nothing and the transferred DNase model adds
real signal.

**The depth confound runs against the result, which makes it a lower bound.** DNase is the
*shallower* input in both cell types: 301.1M usable reads against ATAC's 545.7M in K562, and
53.8M against 571.4M in GM12878 (Report 1). GM12878's DNase is also the weaker library by its
own inter-replicate count ceiling, 0.445 against K562's 0.925. Before running this I flagged
the GM12878 arms as confounded and said I would not read an assay conclusion off them. The
confound is real and it is pointed the wrong way for the conclusion: DNase wins by **+0.086**
in the cell type where it is 10.6x shallower, against **+0.037** where it is only 1.8x
shallower. Depth-matching would be expected to widen the gap, not close it.

**Consequence for the converter.** The premise it needed is now measured rather than assumed:
DNase is the better representation to be in, the advantage survives transfer, and DNase has a
reproducible base-resolution profile for a converter to target (Report 1). All three had to
hold and all three do.

**Method.**

- Each scoring entry carries its own accessibility bigwig, so a DNase-trained model is always
  fed DNase and an ATAC-trained model always ATAC, transferred included. The comparison is
  assay-versus-assay end to end rather than a model being handed the wrong input.
- `1.22` and `1.23` are `1.11` and `1.12` with exactly two changes each, the accessibility
  bigwig and the output directory, verified by diff on the comment-stripped scripts.
- The transfer arm is scored against GM12878's own ATAC-only model, which is the floor a
  deployed model has to beat to be worth anything.

<details>
<summary>Full methods &amp; code</summary>

The DNase inputs are `genomecov -bg -5` unstranded, matching how `atac_5p.bw` was built, so
the swap changes the assay and nothing else. Replicates are pooled by merging the filtered
BAMs and calling `genomecov` once; an earlier version summed per-replicate bedGraphs through
`bedtools merge`, which collapses bookended intervals and produced a K562 track with 34x the
correct signal that exited 0 and looked valid. The build now asserts that the track sum equals
the main-chromosome read count exactly, with the contig list read from the same chrom-sizes
file the track filters on.

```bash
sbatch scripts/0.32.make_dnase_5prime.sh
for F in 0 1 2 3 4; do
    sbatch scripts/1.22.submit_training_dnase_k562.sh multimodal $F
    sbatch scripts/1.23.submit_training_dnase_gm12878.sh multimodal $F
done
sbatch scripts/2.29.submit_dnase_swap_eval.sh
```
Source: `scripts/0.32`, `1.22`, `1.23`, `2.29`
"""
...
# --- Report 4: the ATAC->DNase converter -------------------------------------
#
# Its own report rather than a section of report 3 because it has a different churn rate and
# a different subject. Report 3 is the workbench for the H3K27ac model; this is a separate
# model with a separate target, and the only thing it shares is the trunk.

R4_DATE = "2026-09-10"

R4_HEAD = """## Headline figure
...
R4_RESULT = """## The converter reaches 95% of the DNase profile ceiling in K562
...
R4_CONTROL = """## Sequence contributes, so this is a conversion and not a rescaling
...
R4_TRANSFER = """## The gain transfers, the performance does not
...
def build(pull, write):
    ...
    write("report4_atac_to_dnase_converter.qmd",
          "An ATAC-to-DNase converter recovers 95% of the DNase profile in K562, and the gain over the ATAC track survives transfer",
          R4_DATE,
          [R4_HEAD, R4_RESULT, R4_CONTROL, R4_TRANSFER, R4_LIMITS, R4_METHODS])
