...
See `CLAUDE.md` for full pipeline documentation (directory layout, numbered scripts, environments, file formats) and `todo/TODOLIST.md` for current work status. This document maps the manuscript figures to the scripts and output files that produced them.
...
**Note on 2a (bottom):** `finemo_tf_correlation.py` computes the correlation and generates a bar chart per motif (`top10_per_motif.pdf`) but does not yet apply the 3-way color code described in the legend (p300/CBP interactor / TF without evidence / other nuclear protein). That 3-category coloring currently exists only in `plot_finemo_composite_figure.py`, which is scoped to 3 example motifs (CTCF, AP-1, GATA) and uses a manually curated `P300_INTERACTORS` set that still needs a literature/BioGRID pass (see `todo/TODOLIST.md`) and a 3rd "other nuclear protein" category. Generalizing that coloring logic to all p300-important motifs is the remaining step to produce the final panel.
