...
## F-015: Giving the profile head DNase instead of H3K27ac makes it learnable and moves the counts head, but not yet by more than the stopping rule explains
**Status:** provisional, pending the epoch-matched control
**Claim:** In K562, swapping the profile head's target from H3K27ac to pooled stranded DNase while the counts head keeps H3K27ac turns a dead auxiliary task into a live one: validation profile Pearson rises from 0.060-0.068 to 0.530-0.561 across all five folds, and the head abandons H3K27ac shape entirely (scored against H3K27ac on held-out elements, `profile_pearson` 0.004 against the baseline's 0.064). The counts head improves by a paired within-fold **+0.0026 [+0.0013, +0.0039]** (*p*=0.0046) overall and **+0.0109 [+0.0006, +0.0211]** (*p*=0.043) on the top quintile, 0.701 against 0.690. Every fold moves in the same direction on both metrics. This needs no DNase at inference: DNase is a training-time target only.
**Implications:** The premise behind the profile head's redesign is confirmed as a premise. H3K27ac's 1 bp inter-replicate ceiling is 0.21 and `profile_pearson` sits at 0.063-0.064 across all four accessibility inputs (F-013), so that head was consuming capacity for a near-zero gradient exactly as D-3's consequences note predicted; DNase at a 0.848 ceiling gives it a real task. But the effect on the quantity of interest is at the edge of resolvable, with a top-quintile lower bound of +0.0006, and it is an order of magnitude smaller than swapping the accessibility INPUT to real DNase (+0.038, F-010). On current evidence this is a real but small architectural gain, not a route to closing the gap the DNase input opens.
**Caveats:** **The arms did not train for the same length, and that alone could explain the win.** Early stopping watches the total validation loss, which for the multi-task arm includes the reweighted DNase profile term, so the two arms stop on different objectives: 53-99 epochs against 32-55, every fold longer. `1.29` runs the baseline to the same 100-epoch budget with checkpoint selection unchanged, and until it reports, "a learnable auxiliary task helps the trunk" and "this arm trained longer" fit the result equally well. Separately, `--profile-loss-weight` was set to 0.0561, the measured H3K27ac/DNase per-window depth ratio (51.1 against 910.2 mean total, 17.8x), so the profile term starts at the baseline's magnitude; this is the smallest defensible weight and a null at a larger one has not been ruled out. `profile_*` columns in the scoring table compare both arms against H3K27ac, so the multi-task arm's drop there is by construction and carries no information.
**Tags:** h3k27ac, dnase, multi-task, auxiliary-target, profile-head, k562, architecture, provisional

### Evidence Ledger
| Date | Run/Session | Dataset | Project | Result | Direction |
|------|-------------|---------|---------|--------|-----------|
| 2026-09-14 | training 43416027-31 (`1.28`), scoring 43428399 (`2.39`) | K562 H3K27ac counts + K562 pooled stranded DNase profile, ATAC input, DNase-derived elements, 5 folds | 2026_0824_H3K27ac_model | validation profile *r* 0.530-0.561 against 0.060-0.068; top-quintile Pearson 0.701 against 0.690, paired +0.0109 (*p*=0.043) | supports |
| 2026-09-14 | control 43440391-96 (`1.29`) | same baseline trained to 100 epochs | 2026_0824_H3K27ac_model | PENDING: separates the auxiliary task from the longer training | pending |
