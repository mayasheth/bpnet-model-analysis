...
`todo/TODOLIST.md`; for pipeline/script reference see `CLAUDE.md`; for figure-to-script
...
GM12878) live in `reference/PUBLICATION_FIGURES.md`, not repeated here to avoid drift between copies.
...
- **Open methodological question:** `EP300_peak_overlap` (1000bp window) vs.
  ...
  , see `todo/TODOLIST.md` for the full list of figures that would need regenerating.
...
- ATAC variant: fully trained (5 folds) and predicted; CV Pearson = 0.785
  (all elements), 0.663 (p300+), see `reference/PUBLICATION_FIGURES.md` for the table.
...
See `todo/TODOLIST.md` for the current prioritized list. The two items this section
used to hold on its own, comparing GM12878 and K562 MoDISCo motifs and confirming the
`REPEAT_G` / `NF2L_NFE` / `ELF` identities, were moved into that list on 2026-09-12.
