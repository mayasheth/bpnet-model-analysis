        ...
        if 0 <= current_pos < seq_len - motif_len:
            ...
            if 'N' in motif_seq_to_insert:
                ...
                for bg_idx in range(num_bg):
                    # Convert background one-hot back to sequence for this position range
                    bg_slice = bg_encoded[bg_idx, current_pos:current_pos+motif_len, :]
                    bg_seq = one_hot_decode(bg_slice)
