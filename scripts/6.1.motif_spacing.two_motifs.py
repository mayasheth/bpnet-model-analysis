...
from motif_exp_utils_multiple import (
    get_model, get_shuffled_peak_sequences, one_hot_encode, pattern_to_string,
    make_model_prediction, get_representative_patterns, get_all_patterns,
...
def parse_args():
    ...
    parser.add_argument("--motif-name", nargs="+", type=str,
                        help="Name of the motif(s) for labeling; either one or two values")
    parser.add_argument("--motif-seq", nargs="+", type=str,
                        help="Motif sequence to insert; either one or two distinct sequences; must match motif-names")
    parser.add_argument("--motif-counts", nargs="+", type=int, default=[2],
                        help="Number of motifs to insert (can specify multiple if only one motif-seq)")
    ...
    if (len(args.motif_name) != len(args.motif_seq)):
        parser.error("Number of motif names and sequences must match.")
    if (len(args.motif_name) not in [1, 2]):
        parser.error("Only provide one or two motif names and sequences.")
    if (len(args.motif_name) > 1) and (args.motif_counts != [2]):
        parser.error("If two motifs provided, motif_counts must equal two.")
...
def process_spacing_chunk(spacing_chunk, shared_data):
    ...
    motif_names = shared_data['motif_name']
    ...
    if len(motif_seq) > 1:
        n = 2
        for pattern in get_all_patterns(len(motif_seq)):
        ...
        for n in motif_counts:
            n_motif_seqs = motif_seq[0] * motif_counts
            for pattern in get_representative_patterns(n):
                pstr = pattern_to_string(pattern)
                for s in spacing_chunk:
                    t_ins = time.perf_counter()
                    enc = insert_motifs_with_orientation_general(bg_encoded, n, s, n_motif_seqs, pattern, seq_len)
                    ins_s = time.perf_counter() - t_ins
                    conditions.append((n, pstr, s, enc.shape[0], ins_s))
                    enc_list.append(enc); sizes.append(enc.shape[0])
    ...
    for idx, (n, pstr, s, enc_len, ins_s) in enumerate(conditions):
        ...
        for mi in range(len(model_info_list)):
            ...
            # Add individual motif comparisons if multiple motifs
            if len(motif_names) > 1:
                for motif_name in motif_names:
                    individual_preds = individual_motif_preds[motif_name][mi]
                    log2_vs_individual = float(np.mean(preds_cond - individual_preds[:preds_cond.shape[0]]) / np.log(2))
                    result[f"log2_fc_vs_single_{motif_name}"] = log2_vs_individual
...
def save_results_with_individual_motifs(df, args, logger):
    """Save results with proper handling of individual motif columns"""
    
    # Identify individual motif columns
    individual_cols = [col for col in df.columns if col.startswith('log2_fc_vs_single_') and col != 'log2_fc_vs_single']
    ...
    if individual_cols:
        print(f"Individual motif comparisons available for: {[col.replace('log2_fc_vs_single_', '') for col in individual_cols]}")
    ...
    # Add individual motif columns to aggregation
    for col in individual_cols:
        agg_cols[col] = ['mean', 'std', 'sem']
    
    df_summary = df.groupby(['motif_counts', 'orientation_pattern', 'spacing']).agg(agg_cols).round(4)
...
def main():
    ...
    logger.info("Computing single motif references...")
    t0 = time.perf_counter()

    # Compute individual motif references for each motif
    individual_motif_preds = {}
    all_individual_preds = []

    for i, motif_seq in enumerate(args.motif_seq):
        motif_name = args.motif_name[i]
        logger.info(f"Computing single motif reference for {motif_name}...")
        individual_inserted = insert_motifs_with_orientation_general(
            bg_encoded, n=1, spacing=0, motif_seq=[motif_seq], orientation_pattern=('+',), seq_len=args.seq_len
        )
    ...
    # Combine individual predictions for the overall single motif reference
    # This represents the expected prediction from the sum of individual motif effects
    if len(args.motif_seq) == 1:
        single_motif_preds = all_individual_preds[0]
    else:
        # Average the individual predictions (additive effect assumption)
        single_motif_preds = np.mean(all_individual_preds, axis=0)
...
# Updated shared_data dictionary
    ...
    df_summary = save_results_with_individual_motifs(df, args, logger)
