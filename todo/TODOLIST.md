...
- [~] **Wider receptive field** — RUNNING. `n_layers` 10, trimming 2093, in-window 5186
      (~4.2 kb vs ~1.1 kb). Sequence and multimodal × 5 folds × K562 and GM12878 submitted;
      ATAC-only held back. Scripts `1.13`/`1.14`, generated by `make_wide_scripts.py` with
      exactly three edits from the accs5p scripts, so `*_accs5p_hw500_clw10` is the matched
      baseline. Motivation: H3K27ac is still at 28% of maximum at ±2 kb.
      **Score on the intersection of valid regions** — the wider window drops peaks within
      2593 bp of a chromosome end that the narrow one keeps.
